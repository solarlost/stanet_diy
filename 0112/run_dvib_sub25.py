...
class DetailedLogger(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        if (epoch + 1) % self.print_every == 0:
            ...
            class_acc = logs.get('class_output_accuracy', 0.0)
            val_class_acc = logs.get('val_class_output_accuracy', 0.0)
            
            ortho_eeg = logs.get('ortho_eeg', 0.0)
            ortho_fnirs = logs.get('ortho_fnirs', 0.0)
            sim_loss = logs.get('sim_loss', 0.0)
            
            print(f"Epoch {epoch+1:03d}: "
                  f"Loss={loss:.4f} | "
                  f"Acc={class_acc:.4f} (Val={val_class_acc:.4f}) | "
                  f"Ortho={ortho_eeg+ortho_fnirs:.6f} | "
                  f"Sim={sim_loss:.6f}")
...
def run_subject(subject_id: int, args: argparse.Namespace):
    ...
    for session in range(3):
        ...
        # 构建 D-Net (No VIB)
        model = dvib_net(latent_dim=args.latent_dim, ortho_weight=args.ortho_weight, sim_weight=args.sim_weight)
        ...
        csv_logger = keras.callbacks.CSVLogger(os.path.join(log_dir, f"training_log_sub{subject_id:02d}_sess{session}_dnet.csv"))
        ...
        print(f"  -> Stage 1 Best Val Loss: {min_loss:.4f} (Epoch {min_epoch})")

        print("  -> Stage 2 Training...")
        callbacks_stage2 = [TargetAccCallback(target_acc_val), DetailedLogger(print_every=20)]
        ...
        print(f"  -> Final Test Acc: {test_results[acc_idx]:.4f}")
    ...
    out_path = os.path.join(results_dir, f"subject_{subject_id:02d}_dnet.json")
...
def main():
    ...
    # D-Net Params
    parser.add_argument("--ortho_weight", type=float, default=1.0, help="Orthogonality weight")
    parser.add_argument("--sim_weight", type=float, default=10.0, help="Similarity weight")
    parser.add_argument("--latent_dim", type=int, default=64)

    args = parser.parse_args()
    
    print(f"🚀 D-Net (No VIB) 测试 (Subject {args.subject})")
    print(f"⚙️ 参数: Ortho={args.ortho_weight}, Sim={args.sim_weight}")
