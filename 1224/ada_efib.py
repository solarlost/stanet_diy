...
# ==============================================================================
# Ada-EF-IB Net: Adaptive Dual-Branch Fusion Network
# ==============================================================================
...
class AdaptiveGate(layers.Layer):
    """
    自适应门控层：根据输入特征决定两个分支的权重
    Output = alpha * Branch1 + (1-alpha) * Branch2
    """
    def __init__(self, name="adaptive_gate"):
        super(AdaptiveGate, self).__init__(name=name)
        self.gate_dense = layers.Dense(1, activation='sigmoid')
        self.alpha_tracker = keras.metrics.Mean(name="gate_alpha")

    def call(self, inputs):
        # inputs: [batch, features]
        # alpha: [batch, 1] (0~1)
        # alpha 接近 1 -> 偏向分支 1 (NoIB)
        # alpha 接近 0 -> 偏向分支 2 (EF-IB)
        alpha = self.gate_dense(inputs)
        self.alpha_tracker.update_state(tf.reduce_mean(alpha))
        return alpha

def ada_efib_net(latent_dim=128, beta=1e-3, align_weight=0.1):
    """
    Ada-EF-IB Net: 双分支自适应融合网络
    """
    ...
    # 此时我们有了三个特征向量: eegfusion_feature, fnirs_feature, eeg_feature
    # 它们将作为两个分支的共同输入

    # ================= 2. 分支 A: 保守派 (NoIB / Standard) =================
    # 直接进行分类预测
    def get_prediction_head(feat_eeg, feat_fnirs, feat_fusion, suffix="noib"):
        # 这里的结构复刻原版 sta_net 的分类头
        p_eegfusion = layers.Dense(2, activation='softmax')(layers.Dense(2)(feat_fusion))
        p_fnirs = layers.Dense(2, activation='softmax')(layers.Dense(2)(feat_fnirs))
        p_eeg = layers.Dense(2, activation='softmax', name=f'eeg_output_{suffix}')(layers.Dense(2)(feat_eeg))
        
        p_eegfusion = expand_dims_layer(axis=1)(p_eegfusion)
        p_fnirs = expand_dims_layer(axis=1)(p_fnirs)
        
        pred_concat = layers.Concatenate(axis=1)([p_eegfusion, p_fnirs])
        
        w_fnirs = layers.Dense(1)(feat_fnirs)
        w_fusion = layers.Dense(1)(feat_fusion)
        w = layers.Concatenate()([w_fusion, w_fnirs])
        w = layers.Activation('softmax')(w)
        w = expand_dims_layer(axis=-1)(w)
        
        final_pred = layers.Multiply()([pred_concat, w])
        final_pred = reduce_sum_layer(axis=1, keepaxis=False)(final_pred)
        return final_pred, p_eeg

    pred_noib, eeg_pred_noib = get_prediction_head(eeg_feature, fnirs_feature, eegfusion_feature, suffix="noib")

    # ================= 3. 分支 B: 激进派 (EF-IB / Denoised) =================
    # 先通过 IB 层，再对齐，再分类
    ...
    # 使用 latent 特征进行预测
    pred_efib, eeg_pred_efib = get_prediction_head(eeg_latent, fnirs_latent, eegfusion_latent, suffix="efib")

    # ================= 4. 自适应门控融合 (Adaptive Gating) =================
    # 门控网络根据原始特征决定信赖哪个分支
    # 将三个特征拼接作为门控的输入
    gate_input = layers.Concatenate()([eeg_feature, fnirs_feature, eegfusion_feature])
    
    # alpha: 0~1. 
    # 我们定义: alpha * NoIB + (1-alpha) * EF-IB
    alpha = AdaptiveGate(name="branch_gate")(gate_input)
    
    # 最终融合预测
    final_pred = layers.Add(name="class_output")([
        layers.Multiply()([pred_noib, alpha]),
        layers.Multiply()([pred_efib, 1 - alpha])
    ])
    
    # 辅助输出 (EEG) 也进行融合
    final_eeg_pred = layers.Add(name="eeg_output")([
        layers.Multiply()([eeg_pred_noib, alpha]),
        layers.Multiply()([eeg_pred_efib, 1 - alpha])
    ])

    model = keras.Model(inputs=[eeg_input, fnirs_input], outputs=[final_pred, final_eeg_pred], name="ada_efib_net")
