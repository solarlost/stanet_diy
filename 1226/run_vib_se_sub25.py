...
def run_subject(subject_id: int, args: argparse.Namespace):
    ...
    for session in range(3):
        ...
        # 构建 VIB-SE-Net
        model = vib_se_net(latent_dim=args.latent_dim, beta=args.beta, gamma=args.gamma)
    ...
    out_path = os.path.join(results_dir, f"subject_{subject_id:02d}_vib_se.json")
...
def main():
    ...
    # VIB Params
    parser.add_argument("--beta", type=float, default=1e-4, help="Weight for KL Loss")
    parser.add_argument("--gamma", type=float, default=1.0, help="Weight for Reconstruction Loss")
    parser.add_argument("--latent-dim", type=int, default=128)

    args = parser.parse_args()
    
    print(f"🚀 VIB-SE-Net 测试 (Subject {args.subject})")
    print(f"⚙️ 参数: Beta={args.beta}, Gamma={args.gamma}, Patience={args.patience}")
