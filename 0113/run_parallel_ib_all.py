...
def run_subject(subject_id: int, args: argparse.Namespace):
    ...
    results_dir = os.path.join(current_dir, "results", "parallel_ib_all")
    ...
    for session in range(3):
        ...
        # 权重保存路径
        weight_path = os.path.join(weights_dir, f"weights_sub{subject_id:02d}_sess{session}.h5")
        ...
        callbacks = [
            ...
            # 新增: 保存最佳权重
            tf.keras.callbacks.ModelCheckpoint(filepath=weight_path, monitor="val_class_output_loss", save_best_only=True, save_weights_only=True, mode="min", verbose=0)
        ]
        ...
        # Stage 2 也需要保存，但要注意不要覆盖 Stage 1 更好的结果
        # 这里我们简单起见，Stage 2 结束后如果效果更好，ModelCheckpoint 会自动保存
        # 但由于 Stage 2 没有验证集，我们通常只保存 Stage 1 的最佳权重，或者保存 Stage 2 的最终权重
        # 策略: Stage 2 主要是微调，我们保存 Stage 2 结束后的权重作为 "final"
        ...
        # 手动保存 Stage 2 结束后的权重 (覆盖之前的，或者另存为 final)
        # 考虑到 Stage 2 是用全量数据训练，理论上泛化性更好，但也可能过拟合
        # 我们这里选择保存 Stage 2 结束后的权重
    ...
    out_path = os.path.join(results_dir, f"subject_{subject_id:02d}_parallel_ib.json")
...
def main():
    ...
    summary = []
    print(f"🚀 MS-SE-Parallel-IB-Net 全员测试启动 (带权重保存)")
    ...
    start_subject = 1
    summary_csv_path = os.path.join(current_dir, "results", "summary_parallel_ib_vs_noib.csv")
    ...
    if summary:
        ...
        avg_parallel = summary_df['parallel_ib_acc'].mean()
        
        print("\n" + "=" * 60)
        print("📊 最终对决: MS-SE-Parallel-IB-Net vs NoIB")
        print("=" * 60)
        print(summary_df.to_string(index=False))
        print("-" * 60)
        print(f"AVERAGE | {avg_noib:.4f}     | {avg_parallel:.4f}     | {avg_parallel - avg_noib:+.4f}")
