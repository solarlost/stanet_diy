...
def run_subject(subject_id: int, args: argparse.Namespace):
    ...
    results_dir = os.path.join(current_dir, "results_ma", "parallel_ib_all")
    ...
    if not os.path.exists(subject_filepath):
        print(f"⚠️ Data not found for Subject {subject_id}: {subject_filepath}")
        return None
...
def main():
    ...
    summary = []
    print(f"🚀 MS-SE-Parallel-IB-Net 全员测试启动 (MA Task)")
    print(f"⚙️ 参数: Beta={args.beta}")
    
    summary_csv_path = os.path.join(current_dir, "results_ma", "summary_parallel_ib_ma.csv")
    ...
    for subject_id in range(1, 30):
        ...
        try:
            ...
            if acc is None: # Skip if data not found
                continue
    ...
    if summary:
        summary_df = pd.DataFrame(summary)
        avg_acc = summary_df['acc'].mean()
        
        print("\n" + "=" * 60)
        print("📊 MS-SE-Parallel-IB-Net (MA Task) 结果")
        print("=" * 60)
