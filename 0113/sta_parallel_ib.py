...
# ==============================================================================
# MS-SE-Parallel-IB-Net: Multi-Scale SE + Parallel IB (Safe Enhancement)
# ==============================================================================
...
class reduce_sum_layer(keras.layers.Layer):
    def __init__(self, axis, keepaxis, name=None, *args, **kwargs):
        super(reduce_sum_layer, self).__init__(name=name, **kwargs)
        self.axis = axis
...
class InformationBottleneck(layers.Layer):
    def __init__(self, latent_dim, beta=1e-5, name=None):
...
def sta_net_parallel_ib(latent_dim=128, beta=1e-5):
    """
    MS-SE-Parallel-IB-Net
    """
    ...
    # 2. 拼接 原始特征 + IB特征
    # 这样分类器可以同时看到“全貌”和“精华”
    eegfusion_combined = layers.Concatenate()([eegfusion_feature, eegfusion_latent])
    fnirs_combined = layers.Concatenate()([fnirs_feature, fnirs_latent])
    eeg_combined = layers.Concatenate()([eeg_feature, eeg_latent])

    # --- Classification ---
    eegfusion_pred = layers.Dense(2)(eegfusion_combined)
    fnirs_pred = layers.Dense(2)(fnirs_combined)
    eeg_pred = layers.Dense(2)(eeg_combined)
    ...
    # 权重计算也使用 Combined 特征
    fnirs_p_weight = layers.Dense(1)(fnirs_combined) 
    eegfusion_p_weight = layers.Dense(1)(eegfusion_combined) 
    ...
    model = keras.Model(inputs=[eeg_input, fnirs_input], outputs=[the_pred, eeg_pred], name="sta_net_parallel_ib")
