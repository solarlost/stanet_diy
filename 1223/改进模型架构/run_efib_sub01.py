...
class BetaScheduler(keras.callbacks.Callback):
    ...
    def on_epoch_begin(self, epoch, logs=None):
        ...
        for layer in self.model.layers:
            # 递归查找子层 (因为我们的 IB 层可能在嵌套的 Model 或 Layer 中)
            if hasattr(layer, "layers"):
                for sublayer in layer.layers:
                    if hasattr(sublayer, "beta"):
                        try:
                            sublayer.beta = float(beta_now)
                        except Exception:
                            pass
            # 直接检查当前层
...
def run_efib_subject(subject_id: int, args: argparse.Namespace):
    # 路径设置
    # 直接硬编码项目根目录，确保万无一失
    # 假设脚本位于 D:/test/stanet_diy/1223/改进模型架构/run_efib_sub01.py
    # 我们需要指向 D:/test/stanet_diy
    
    # 向上回退 3 层: 改进模型架构 -> 1223 -> stanet_diy
    ...
    print(f"📂 尝试读取数据: {subject_filepath}")
    
    if not os.path.exists(subject_filepath):
        # 如果还是找不到，尝试绝对路径硬编码 (作为最后的保险)
        subject_filepath = f"D:/test/stanet_diy/data/model_input/{subject_file}"
        print(f"⚠️ 相对路径失败，尝试绝对路径: {subject_filepath}")

    if not os.path.exists(subject_filepath):
        raise FileNotFoundError(f"❌ 无法找到数据文件: {subject_filepath}")
    ...
    for session in range(3):
        print(f"\n--- Session {session} [Subject {subject_id:02d}] ---")
        ...
        print(f"🔨 构建 EF-IB Net (Beta={args.beta}, AlignWeight={args.align_weight})...")
        ...
        callbacks = [
            tf.keras.callbacks.EarlyStopping(
                monitor="val_class_output_loss", patience=args.patience, restore_best_weights=True, verbose=0, mode="min"
            ),
        ...
        print(f"  -> Stage 1 Best Loss: {min_loss:.4f} (Epoch {min_epoch})")

        # 第二阶段训练
        print("  -> Stage 2 Training...")
        callbacks_stage2 = [
            TargetAccCallback(target_acc_val),
            BetaScheduler(args.beta_start, args.beta, args.beta_warmup_epochs)
        ]
        ...
        # test_results 包含: [total_loss, class_loss, eeg_loss, class_acc, eeg_acc, ...]
        # 注意: 因为加了 alignment loss，total_loss 会包含它
        
        # 打印结果 (假设 class_acc 是第 4 个指标，index 3)
        # 我们可以通过 model.metrics_names 来确认
        acc_idx = model.metrics_names.index('class_output_accuracy')
        loss_idx = model.metrics_names.index('class_output_loss')
        
        acc = test_results[acc_idx]
        loss = test_results[loss_idx]
        
        print(f"  -> Result: Acc {acc:.4f} | Class Loss {loss:.4f}")
    ...
    with open(out_path, "w", encoding="utf-8") as f:
        json.dump({
            ...
            "params": vars(args),
            "metrics_names": ["total_loss", "class_loss", "eeg_loss", "class_acc", "eeg_acc"] # 近似值，具体看 log
        }, f, indent=2)

    return out_path, mean_results[3] # 返回 Acc
...
def main():
    ...
    # 新参数: 对齐权重 (从 0.1 降低到 0.01)
    parser.add_argument("--align-weight", type=float, default=0.01, help="Weight for cross-modal alignment loss")

    args = parser.parse_args()

    print(f"🚀 启动 EF-IB Net 测试 (Subject 01)")
    print(f"⚙️ 参数: Beta={args.beta}, AlignWeight={args.align_weight}")
    
    try:
        out_path, acc = run_efib_subject(1, args)
        print(f"\n✅ 测试完成!")
