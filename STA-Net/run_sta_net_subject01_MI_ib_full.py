...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train STA-Net with Information Bottleneck on subject_01 MI dataset using all sessions jointly."
    )
    parser.add_argument(
        "--metrics-out",
        type=str,
...
def main():
    ...
    print("开始训练第一个被试（MI）数据集 - 信息瓶颈版本（全量训练）")
    ...
    print("begin first train (information bottleneck, few-shot)")
    ...
    print("begin second train (information bottleneck, full data)")
    model.fit(
        full_train_dataset,
    ...
    print(f"验证/测试结果（信息瓶颈，全量训练）: {metrics}")
