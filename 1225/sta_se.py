...
# ==============================================================================
# SE-STA-Net: STA-Net with Squeeze-and-Excitation Attention Blocks
# ==============================================================================
...
class SEBlock(layers.Layer):
    ...
    def call(self, inputs):
        # inputs shape: (batch, d1, d2, d3, channels)
        # Squeeze: 全局平均池化
        x = self.avg_pool(inputs)
        # Excitation: 两个全连接层学习权重
        x = self.fc1(x)
        x = self.fc2(x)
        # Scale: 将权重应用回原始特征
        x = self.reshape(x)
...
class conv_block_se(keras.layers.Layer):
    ...
    def __init__(self, eeg_filter, eeg_size, eeg_stride,
        ...
        self.eeg_se = SEBlock(channels=eeg_filter, name="eeg_se") # Add SE
        ...
        self.fnirs_se = SEBlock(channels=fnirs_filter, name="fnirs_se") # Add SE
        ...
        self.eegfusion_se = SEBlock(channels=eegfusion_filter, name="fusion_se") # Add SE
    ...
    def call(self, inputs):
        ...
        eeg_feature = self.eeg_se(eeg_feature) # Apply SE
        ...
        fnirs_feature = self.fnirs_se(fnirs_feature) # Apply SE
        ...
        eegfusion_feature = self.eegfusion_se(eegfusion_feature) # Apply SE
...
def sta_net_se():
    ...
    model = keras.Model(inputs=[eeg_input, fnirs_input], outputs=[the_pred, eeg_pred], name="sta_net_se")
