...
from sta_se_ib import sta_net_se_ib
...
def run_subject(subject_id: int, args: argparse.Namespace):
    ...
    for session in range(3):
        ...
        # 构建 SE-IB-STA-Net
        model = sta_net_se_ib(latent_dim=args.latent_dim, beta=args.beta, align_weight=args.align_weight)
    ...
    out_path = os.path.join(results_dir, f"subject_{subject_id:02d}_se_ib.json")
...
def main():
    ...
    print(f"🚀 SE-IB-STA-Net 测试 (Subject {args.subject})")
    print(f"⚙️ 参数: Beta={args.beta}, Align={args.align_weight}, Patience={args.patience}")
